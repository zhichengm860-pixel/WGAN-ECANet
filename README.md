...
- **Parameter Efficient**: Model has approximately 4.4M parameters with ~8.2ms inference latency
...
│   │   ├── __init__.py
│   │   └── dataloader.py            # Unified data loader (supports streaming & standard)
│   ├── models/                      # Model definitions
│   │   ├── __init__.py
│   │   └── wgan_ecanet.py          # WGAN-ECANet model
│   ├── training/                    # Training modules
│   │   ├── __init__.py
│   │   └── trainer.py              # WGAN-GP trainer
│   └── utils/                       # Utility functions
│       ├── __init__.py
│       ├── metrics.py               # Evaluation metrics (including SNR analysis)
│       └── path_manager.py          # Path management
│
├── dataset/                         # Dataset documentation
│   └── dataset_explanation.md       # Dataset description
│
├── train.py                         # Full WGAN-GP training script
├── evaluate.py                      # Model evaluation with SNR analysis
...
| File | Description |
|------|-------------|
| `train.py` | Full WGAN-GP adversarial training with classifier |
| `evaluate.py` | Full evaluation with SNR analysis and generator quality |
| `src/models/wgan_ecanet.py` | Core WGAN-ECANet model (Generator, Discriminator, Classifier) |
| `src/training/trainer.py` | WGAN-GP training loop with gradient penalty |
| `src/data/` | Data loaders for RadioML datasets |
| `src/utils/metrics.py` | Evaluation metrics including SNR-based analysis |
...
The project supports RadioML datasets (download separately):

- **RadioML 2018.01A**: https://www.deepsig.io/datasets
- Place at: `dataset/RadioML 2018.01A/GOLD_XYZ_OSC.0001_1024.hdf5`
...
**Full WGAN-GP Training (Complete):**
```bash
# Complete WGAN-GP adversarial training
python train.py --mode wgan --epochs 100

# With dynamic gradient penalty
python train.py --mode wgan --dynamic_gp --epochs 100

# Resume from checkpoint
python train.py --mode wgan --resume results/wgan_training_xxx/checkpoint.pth

# Classifier-only mode (faster, no GAN)
python train.py --mode classifier --epochs 50
```

Training time: ~2 hours on dual V100 GPUs (classifier only), ~4 hours (full WGAN).

#### 2. Evaluation

```bash
# Full evaluation with SNR analysis
python evaluate.py --model results/wgan_training_xxx/best_model.pth \
    --dataset dataset/RadioML\ 2018.01A/GOLD_XYZ_OSC.0001_1024.hdf5 \
    --evaluate-generator
```
...
This project is designed with a modular architecture, supporting custom training and model extension.

### Custom Training

#### 1. Create New Training Scripts

Create custom training scripts in the project root directory, following these steps:

**Step 1: Import necessary modules**
```python
import torch
from src.models.wgan_ecanet import WGANECANet
from src.data import RadioMLDataLoader, create_dataloaders
...
model = WGANECANet(num_classes=24, use_spectral_norm=True)
...
To modify model architecture, edit `src/models/wgan_ecanet.py`:
...
```bash
export WGAN_MODEL_PATH=results/wgan_training_xxx/best_model.pth
export WGAN_DATASET_PATH=dataset/RadioML 2018.01A
export WGAN_DEVICE=cuda
```

#### Method 3: Command Line Arguments

```bash
python evaluate.py --model results/wgan_training_xxx/best_model.pth --dataset dataset/RadioML\ 2018.01A/GOLD_XYZ_OSC.0001_1024.hdf5
```
...
1. **Dataset Dependency**
   ...
   - Training examples use RadioML 2018.01A
   - Cross-dataset usage requires retraining
...
5. **Model Compatibility**
   - Model requires `use_spectral_norm=True`
   - Input signals must be I/Q dual-channel format
   - Architecture modifications require retraining
...
- Validate training process completed successfully
